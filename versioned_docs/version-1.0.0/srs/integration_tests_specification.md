...
| :--- | :--- | :--- | :--- | :--- |
| **1. Golden Master Regression** | Verifies parsing and rendering consistency against portable baselines. | **Partially Confirmed** — `engine/tests/test_golden_master.py` implemented ✅; `Tests/run_regression_tests.py` **location unconfirmed** ⚠️ (GAP-31) | **Already Implemented** (Parser & Renderers) | `engine/tests/test_golden_master.py` ✅ <br/> `Tests/run_regression_tests.py` ⚠️ <br/> `REQ-FUN-48` |
| **2. Docomatic Alignment** | Ensures semantic and ToC alignment with legacy Docomatic outputs. | **Implemented** ✅ | **Already Implemented** (ToC & Structure Mapping) | `engine/tests/test_docomatic_alignment.py` ✅ <br/> `REQ-FUN-49` |
| **3. Page Integrity Verifier** | Ensures compiled pages physically exist on disk and contain correct heading signatures. | **Location Unconfirmed** ⚠️ (GAP-31) | **Already Implemented** (HTML Page Layouts) | `Tests/verify_pages.py` ⚠️ <br/> `REQ-FUN-31`, `REQ-FUN-32` |
| **4. Post-Build Link Checker** | Crawls generated HTML to validate local pathways and external URLs. | **Location Unconfirmed** ⚠️ (GAP-31) | **Already Implemented** (Routing & Delimiter Layouts) | `Tests/check_links.py` ⚠️ <br/> `REQ-FUN-31` |
| **5. Incremental Build Integrity** | Validates that changes to source files write only the affected compiled assets. | *Planned* (v2.0) | **Already Implemented** (`BuildCacheManager`) | `test_incremental_build.py` (Planned) <br/> `REQ-FUN-29`, `TSK-DAT-03` |
| **6. Multi-Language Cross-Link Resolver** | Verifies cross-reference resolution for mixed-language APIs (C++, C#, Python). | **Location Unconfirmed** ⚠️ (bundled in `Tests/check_links.py`) | **Already Implemented** (Polymorphic Delimiters) | `Tests/check_links.py` ⚠️ <br/> `REQ-FUN-31`, `TSK-RND-09` |
| **7. RAG-Friendly Export Schema Validator** | Validates structure, metadata coverage, and schema compliance of exported JSON datasets. | *Planned* (v3.0+, depends on RAG feature) | **Already Implemented** (Pydantic IR Schema & Gzip Storage) | `test_rag_schema.py` (Planned) <br/> `REQ-FUN-05`, `REQ-BUS-04` |
| **8. Robustness against Doxygen Versions** | Runs parser over pre-defined XML schemas from various Doxygen releases to ensure compatibility. | *Planned* (v2.0) | *Planned* (Cross-Version Parser) | `test_doxygen_compatibility.py` (Planned) <br/> `REQ-FUN-19`, `TSK-PAR-02` |
| **9. API Coverage Audit** | Analyzes the ratio of documented code entities and alerts on undocumented structures. | *Planned* (v2.0, depends on GAP-09) | **Already Implemented** (Docstring Extraction & IR Metadata) | `test_api_coverage.py` (Planned) <br/> `REQ-FUN-48` |
...
| **11. Search Index Validity** | Validates that compiled search engine index JSON keys match physical page anchors. | *Planned* (v3.0+) | **Already Implemented** (Search Engine Indexing) | `test_search_index.py` (Planned) <br/> `REQ-FUN-31` |
| **12. Wrapper Boundary Integrity** | Verifies parameter mapping consistency across multi-language API wrappers (SWIG/JNI). | *Planned* (v3.0+, depends on SWIG-aware parser) | **Already Implemented** (SWIG Wrappers & Core Parsing) | `test_boundary_integrity.py` (Planned) <br/> `TSK-RND-09` |

> ⚠️ **GAP-31 — External Integration Scripts (Unconfirmed Location)**: The three `Tests/` scripts (`run_regression_tests.py`, `verify_pages.py`, `check_links.py`) are referenced in this specification but their location in the repository was **not confirmed** during the 2026-06-28 testing audit. Neither a `pipeline/tests/` nor a `pipeline/loadtest/` top-level directory exists. The in-engine equivalents (`test_golden_master.py`, `test_docomatic_alignment.py`) are confirmed. **v2.0 action**: locate all three scripts or re-implement as pytest-compatible integration tests.
...
*   **Prerequisite (v3.0+)**: Stable HTML template architecture — the v2.0 `sidebar.toml` refactor and custom page rendering (GAP-06, GAP-26) must be complete before screenshot baselines are meaningful.
*   **Language Feasibility Matrix**:
...
*   **Prerequisite (v3.0+)**: The v3.0+ SWIG wrapper-aware parser (Section 6 of `requirements_v3_deferred.md`) must be implemented first for this test to have meaningful source-of-truth data.
*   **Language Feasibility Matrix**:
...
## 🧪 Open Architectural Items (Technical Debt)

### GAP-31 — External Integration Script Location Unconfirmed

During the 2026-06-28 testing audit, the following three scripts referenced throughout this specification were **not located** in the repository:

| Script | Spec Reference | Status |
|---|---|---|
| `Tests/run_regression_tests.py` | TEST-INT-01, `REQ-FUN-48` | Location unconfirmed — no `pipeline/tests/` directory exists |
| `Tests/verify_pages.py` | TEST-INT-03, `REQ-FUN-31`/`REQ-FUN-32` | Location unconfirmed |
| `Tests/check_links.py` | TEST-INT-04, TEST-INT-06, `REQ-FUN-31` | Location unconfirmed |

The in-engine test suite (`engine/tests/`) contains confirmed implementations covering the same scope:
* `test_golden_master.py` — covers TEST-INT-01 scope (golden master regression)
* `test_docomatic_alignment.py` — covers TEST-INT-02 scope (Docomatic alignment)

**v2.0 Action**: Locate all three `Tests/` scripts in the repository, commit them if they exist in an untracked state, or re-implement them as pytest-compatible integration tests within `engine/tests/`.

---

### GAP-32 — Per-Language Integration Test Suites Missing

Current test coverage per language is limited to golden master regression (structure-level comparison). Each language requires a dedicated integration test suite exercising the **full parse → render pipeline** from Doxygen XML input through to output file structure verification.

Missing coverage per language (independent of v3.0+ render parity work — GAP-28):
...
Each suite should exercise the full parse → render cycle for the target language using `engine/tests/assets/doxygen/` fixture XMLs (or augmented per-language fixtures) and verify the presence and structural correctness of all currently generated page types. These suites are **independent** of the v3.0+ language parity work (GAP-28) — they validate existing output structure, not new rendering features.

**Traced Tasks**: `TSK-RND-06` (empty group pruning), `TSK-RND-07` / `TSK-RND-08` (static/inline/redirect pages — defer to v3.0+)
