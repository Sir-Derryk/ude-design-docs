...
## v1.0 Command Structure (Flat CLI)

In v1.0, the CLI is a single flat command accepting three required arguments. There are no subcommands.
...
|---|---|
| `--global-config` | Path to `ude_global_config.json` (global defaults) |
| `--sdk-config` | Path to `ude_sdk_config.json` (SDK-level overrides) |
| `--doc-config` | Path to `ude_doc_config.json` (per-target overrides) |

The CLI is fully non-interactive and suitable for use in CI/CD environments (REQ-BUS-09).

---

## v2.0 Planned Command Structure (Subcommands)

:::info Planned for v2.0 (GAP-01)
The subcommand structure below is **not implemented in v1.0**. It is the target architecture for v2.0, dependent on the UdeOrchestrator library API (GAP-05).
:::

```text
ude
├── compile    # Full pipeline: collect → parse → render (equivalent to v1.0 flat invocation)
├── parse      # Collect and parse only; serialize IR to L1 cache (.json.gz)
├── render     # Load IR from L1 cache; render only (skip collection and parsing)
└── audit      # Generate documentation coverage report (new in v2.0)
```

`ude parse` followed by `ude render` must produce identical output to `ude compile`.

### `ude compile`
Executes the full pipeline: ingests raw analysis data, validates IR, and renders pages in a single step.

* **Arguments**:
  * `--global-config`: Path to global configuration file.
  * `--sdk-config`: Path to SDK-level configuration file.
  * `--doc-config`: Path to target-specific configuration file.

### `ude parse`
Performs the frontend ingestion phase only, saving the validated IR to a compressed file.

* **Arguments**:
  * `--global-config`, `--sdk-config`, `--doc-config`: Same as `ude compile`.

### `ude render`
Performs the backend rendering phase only, reading the compressed IR from the L1 cache.
...
| Field | v1.0 Status | Description |
|---|---|---|
| `error_policy` | **Active** | Multi-target orchestration fault tolerance. `fail-fast` aborts at first project failure; `continue-on-error` logs, skips, and completes remaining targets. |
| `doxygen_path` | Reserved (v2.0) | Absolute path or environment lookup for the `doxygen` binary. Takes precedence over `shutil.which()` when set. |
| `log_level` | Reserved (v2.0) | Logging verbosity: `DEBUG`, `INFO`, `WARNING`, `ERROR`. Applied via `logging_setup()` (GAP-12). |
| `log_file` | Reserved (v2.0) | Output filepath for unified thread-safe file logging. Activates `FileHandler` when set. |
| `cache_root_dir` | Reserved (v2.0) | Root directory for L1 parse cache and L2 render cache storage. Wired with `cache_root_dir` activation in GAP-09. |
| `global_templates_dir` | Reserved (v2.0) | Directory for global renderer Jinja2 templates. |
| `translation_service` | Reserved (v3.0+) | LLM provider, model, and authentication configuration for translation requests. |

All reserved fields are accepted by the config parser without error in v1.0 but have no effect. They will be activated in v2.0 (GAP-09) except `translation_service` which activates in v3.0+.
...
*   **`collector`**: Configures the preprocessing stage:
    ...
    *   `doxyfile_template`: Relative path to the Doxygen configuration template file (GAP-08: the key name `doxyfile` used in earlier drafts is incorrect and must not be used).
