...
gantt
    ...
    TSK-PAR-03 (Multi-Language Facade)     :t7_1, after t7, 2d
    TSK-COL-01 (Doxygen XML Collector)     :t8, after t7_1, 2d
    ...
    TSK-RND-09 (Signature Strategies)       :t12_1, after t12, 2d
    TSK-RND-10 (Layout Fallbacks)          :t12_2, after t12_1, 2d
    section Week 5: CLI & Integration
    TSK-CLI-01 (CLI Core & exit codes)     :t13, after t12_2, 2d
...
*   **Objectives**: Create abstract module contracts, implement secure pre-processing collectors, parse Doxygen XML into unified IR, and expose a backward-compatible parser facade routing system.
*   **Deliverables**:
    ...
    *   **Parser Facade**: Unified multi-language routing parser facade (`TSK-PAR-03`).
...
*   **TDD Checkpoint**: Green unit tests asserting proper interface structures, subprocess executions, directory guard rails, XML schema mapping for C++ structs, C# automatic properties, Java packages, and Python classes, along with correct delegation routing inside the parser facade.
...
*   **Objectives**: Implement template compile rules for Hugo markdown and Direct static HTML pages, utilizing extensible signature formatting strategies and fault-tolerant layout fallback loaders.
*   **Deliverables**:
    ...
    *   **Signature Strategies**: Extensible Strategy Pattern for language-specific signatures (`TSK-RND-09`).
    *   **Layout Fallbacks**: Robust dual-stage layout loader with inline fail-safe fallback (`TSK-RND-10`).
*   **TDD Checkpoint**: Green unit tests verifying correct Markdown and HTML syntax generation, front-matter template variables injection, correct language scope delimiters formatting, and fault-tolerant in-memory template loading under file absence.
