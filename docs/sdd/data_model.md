...
:::caution v1.0 vs v2.0 Implementation Status
The **v1.0 implementation** uses a simplified discriminated-union model — all entities are represented as `ClassEntity` instances with an `entity_type: str` field, and `ProjectCatalog` carries `(namespaces, metadata)`. The fully typed 7-model schema below is the **v2.0 target** (GAP-03). The v1.0 actual model is documented in the [v1.0 Implementation section](#v10-actual-implementation) below.
:::

---

## v1.0 Actual Implementation

The v1.0 codebase uses the following simplified Pydantic model, implemented in `engine/ude/models.py`:
...
class ClassEntity(BaseModel):
    """Unified entity model for all API entity types in v1.0."""
    entity_type: str          # "class", "struct", "interface", "enum",
                              # "function", "variable", "typedef"
    name: str
    fully_qualified_name: str
    description: str = ""
    # ... additional fields common to all entity types
...
class ProjectCatalog(BaseModel):
    # v1.0: no project_name or version fields
    namespaces: List[NamespaceEntity] = Field(default_factory=list)
    metadata: Dict[str, Any] = Field(default_factory=dict)
```

`ProjectCatalog` does **not** include `project_name` or `version` fields in v1.0. Separate typed entity models (`EnumModel`, `VariableModel`, `ConstantModel`, `TypeAliasModel`) are **not** part of v1.0. Both are scheduled for v2.0.

---

## v2.0 Target Data Model

The v2.0 data model replaces the `ClassEntity` discriminated union with seven typed Pydantic models and adds `project_name` and `version` to `ProjectCatalog`. The core data structure is defined using **Pydantic v2** models to guarantee fast, typed validation, serialization, and transparent compression:
